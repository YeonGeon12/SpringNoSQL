...
public class AbstractMongoDBComon {
    ...
    protected boolean createCollection(MongoTemplate mongodb, String colNm, String[] index) {
        ...
        // 기존에 등록된 컬랙션 이름이 존재하는지 체크하고, 컬랙션이 없는 경우 생성함
        if (!mongodb.collectionExists(colNm)) {

            // 인덱스 값이 존재한다면...
            if (index.length > 0) {

                // 컬랙션 생성 및 인덱스 생성, MongoDB에서 데이터 가져오는 방식에 맞게 인덱스는 반드시 생성하자!
    ...
    /**
     * 인덱스 컬럼이 한 개일때 컬랙션 생성
     *
     * @param mongodb 접속된 MongoDB
     * @param colNm 생성할 컬랙션 명
     * @param index 생성할 인덱스
     * @return 생성결과
     */
    protected boolean createCollection(MongoTemplate mongodb, String colNm, String index) {
